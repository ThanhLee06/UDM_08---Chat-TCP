...
class RetryIntegrationTest {
    @Test
    void retryAcknowledgesStoredMessageWithoutRedeliveringIt() throws Exception {
        ...
        try (Peer alice = new Peer(server.getPort()); Peer bob = new Peer(server.getPort())) {
            ...
            assertEquals(message.messageId, bob.read(MessageType.CHAT).messageId);
            alice.send(message); alice.read(MessageType.CHAT_OK);
            bob.socket.setSoTimeout(350);
            assertThrows(java.net.SocketTimeoutException.class, () -> bob.read(MessageType.CHAT));
            message.content = "Nội dung khác";
            alice.send(message);
            alice.read(MessageType.ERROR);
